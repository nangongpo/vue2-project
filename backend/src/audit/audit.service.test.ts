...
describe('AuditService', () => {
  it('does not expose internal BigInt actorId in audit detail', async () => {
    ...
    expect(result.data.actor).toEqual({ userId: 'public-user-1', username: 'admin', displayName: '管理员' })
  ...
  it('returns a paginated audit result with filters', async () => {
    ...
    const result = await service.page({ keyword: 'users', result: 'FAILURE', from, page: 2, pageSize: 10 })

    expect(result.data).toEqual({ items: [{ id: 'log-1' }], total: 1, page: 2, pageSize: 10 })
    expect(count).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ result: 'FAILURE', createdAt: { gte: from } }) }))
