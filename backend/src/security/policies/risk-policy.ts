...
export function maxRiskLevel(...levels: RiskLevel[]): RiskLevel {
  return levels.reduce((highest, level) => (riskOrder[level] > riskOrder[highest] ? level : highest), 'L0' as RiskLevel)
}
...
const PERMISSION_RISK_RULES: readonly RiskRule[] = [
  ...
  { level: 'L3', exact: ['system.user.grant', 'system.user.disable', 'system.user.unlock', 'system.user.reset-password', 'system.user.mfa-reset'] },
  { level: 'L2', prefixes: ['system.session.', 'system.auth.', 'auth.password.', 'auth.mfa.', 'auth.reauth.'] },
...
export function resolveRiskLevel(input: { permissions?: string[]; operation?: string; minimumLevel?: RiskLevel }): RiskLevel {
  return maxRiskLevel(
...
export function assertOperationSecurityProof(actor: SecurityProofActor, operation: string, now = new Date()) {
