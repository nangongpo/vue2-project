...
function context(redis: any, options: any) {
  ...
  const request = { method: 'POST', url: '/auth/login', routeOptions: { url: '/auth/login' }, ip: '127.0.0.1', body: { username: 'admin' }, headers: {} }
  return {
...
describe('IdempotencyGuard', () => {
  it('rejects a duplicate request while the lock is held', async () => {
    ...
    await expect(guard.canActivate(context(redis, { scope: 'auth.login' }))).rejects.toMatchObject({ status: 409, response: { code: '100009' } })
