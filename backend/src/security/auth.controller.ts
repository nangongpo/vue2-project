...
import { AuthService, SESSION_COOKIE, sessionCookieOptions } from './auth.service.js'
import { AuthGuard } from './auth.guard.js'
import { API_CODE } from '../common/api-code.js'
import { AuditAction } from '../audit/audit.decorator.js'
import { Idempotent } from './idempotency.decorator.js'
...
export class AuthController {
  ...
  async login(@Body() body: LoginDto, @Req() request: FastifyRequest, @Res({ passthrough: true }) response: FastifyReply) {
    ...
    const result = await this.auth.login(body.username, body.password, request.ip, request.headers['user-agent'], body.captchaToken, body.attemptId)
  ...
  sessions(@Req() request: FastifyRequest) {
    ...
    return this.auth.listSessions(user.internalId, request.cookies?.[SESSION_COOKIE]).then(data => ({ code: API_CODE.SUCCESS, message: 'success', data }))
  }
  ...
  me(@Req() request: FastifyRequest) {
    const user = (request as FastifyRequest & { user: Record<string, unknown> & { internalId?: string; isSuperAdmin?: boolean } }).user
    const { internalId: _internalId, isSuperAdmin: _isSuperAdmin, ...safeUser } = user
    return { code: API_CODE.SUCCESS, message: 'success', data: { ...safeUser, loginIp: request.ip, serverTime: new Date().toISOString() } }
  }
