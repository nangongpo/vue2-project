  ...
  <div class="approval-page">
        ...
        <template v-if="detail">
          <el-descriptions class="approval-detail" :column="1" border>
            ...
            <el-descriptions-item label="类型 / 状态">
              {{ kindLabel(detail.kind) }} / {{ statuses[detail.status]}}
    ...
    <el-dialog title="新建受控审批申请" :visible.sync="createVisible" width="720px" :close-on-click-modal="false">
      ...
      <el-form :model="form" label-width="115px">
        <el-form-item label="申请类型"><el-select v-model="form.kind"><el-option v-for="kind in kinds" :key="kind.value"
              :label="kind.label" :value="kind.value" /></el-select></el-form-item>
        <el-form-item v-if="['ROLE_GRANT', 'ROLE_REVOKE', 'MFA_RESET'].includes(form.kind)" label="用户 UUID"><el-input
            v-model.trim="form.userId" placeholder="公开 userId，不使用数据库数字 ID" /></el-form-item>
        <el-form-item v-if="!['API_ROUTE_CHANGE', 'ELEVATED_REVOKE', 'MFA_RESET'].includes(form.kind)"
          label="角色 UUID"><el-input v-model.trim="form.roleId" placeholder="公开 roleId" /></el-form-item>
        ...
        <el-form-item label="到期时间"><el-date-picker v-model="form.expiresAt" type="datetime"
            placeholder="必填，未来 24 小时内" /></el-form-item>
      </el-form>
