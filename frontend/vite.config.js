...
function resolve(dir) {
  return path.join(import.meta.dirname, dir)
}
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [
      ...
      // 生成文件路径 dist/static/spritemap.svg
    ...
    oxc: {
      compress: {
        dropConsole: true, // 移除所有 console.*
        dropDebugger: true,    // 移除 debugger
        treeshake: {
          manualPureFunctions: ["console.log"],
        },
        mangle: true  // 开启变量名混淆（默认已开启）
      },
    },
    optimizeDeps: {
      exclude: ['element-ui']
    ...
    build: {
      ...
      chunkSizeWarningLimit: 1000, // 以 kB 为单位
