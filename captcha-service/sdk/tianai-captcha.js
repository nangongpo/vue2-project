...
var TianaiCaptcha = (function () {
  ...
  function style() {
    ...
    node.id = styleId
    node.textContent = '.tac{width:100%;font:14px Arial,sans-serif;color:#8a99ad;user-select:none;-webkit-user-select:none}.tac__canvas{position:relative;width:100%;height:120px;overflow:hidden;background:#141c2e;box-shadow:inset 0 0 0 1px #233554}.tac__background{display:block;width:100%;height:100%}.tac__piece{position:absolute;left:0;z-index:2;width:44px;height:44px;pointer-events:none}.tac__track{position:relative;height:42px;margin-top:8px;line-height:42px;background:#141c2e;box-shadow:inset 0 0 0 1px #233554;text-align:center;overflow:hidden}.tac__progress{position:absolute;z-index:2;left:0;top:0;height:100%;width:0;background:linear-gradient(90deg,#15506b,#087f99,#00c4d8);box-shadow:inset -1px 0 0 rgba(0,242,254,.75)}.tac__text{position:relative;z-index:1;color:#a9c7d6;font-weight:500;letter-spacing:.2px}.tac__button{position:absolute;z-index:4;left:0;top:0;width:42px;height:42px;margin-top:-1px;margin-left:-1px;border:1px solid #00f2fe;background:#00f2fe;color:#060913;line-height:42px;text-align:center;cursor:grab;box-sizing:border-box}.tac__button:active{cursor:grabbing}.tac__button.ok{background:#67c23a;border-color:#67c23a;color:#fff;cursor:default}.tac__message{height:18px;margin-top:4px;line-height:18px}.tac__message.error{color:#f56c6c}.tac__message.success{color:#67c23a}'
  ...
  function create(container, options) {
    ...
    function setLeft(value) {
      left = Math.max(0, Math.min(maxLeft(), value)); button.style.left = left + 'px'; progress.style.width = Math.min(track.clientWidth, left + button.offsetWidth) + 'px'; piece.style.left = (left - 2) + 'px'
    }
    ...
    function setChallenge(value) { var payload = value && value.Payload ? value.Payload : value; challenge = payload ? Object.assign({}, value, payload, { challengeId: value.ChallengeId || payload.challengeId, expiresIn: value.ExpiresIn || payload.expiresIn }) : null; reset(); if (!challenge) return; var pieceSize = challenge.pieceSize || 44; var piecePadding = 2; background.onload = function () { emit('RESOURCE_LOADED', { resource: 'background' }) }; background.onerror = function () { emit('RESOURCE_LOAD_FAILURE', { resource: 'background', reason: 'BACKGROUND_LOAD_FAILED' }) }; piece.onload = function () { emit('RESOURCE_LOADED', { resource: 'puzzle' }) }; piece.onerror = function () { emit('RESOURCE_LOAD_FAILURE', { resource: 'puzzle', reason: 'PUZZLE_LOAD_FAILED' }) }; background.src = challenge.backgroundImage; piece.src = challenge.puzzleImage; piece.style.width = (pieceSize + piecePadding * 2) + 'px'; piece.style.height = (pieceSize + piecePadding * 2) + 'px'; piece.style.top = ((challenge.targetY || 0) - piecePadding) + 'px'; piece.style.left = -piecePadding + 'px'; messageText('', '') }
