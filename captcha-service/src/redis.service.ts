...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  private async connect() {
    ...
    const client = new RedisClient(this.redisUrl, { lazyConnect: true, maxRetriesPerRequest: 1, enableOfflineQueue: false, retryStrategy: times => Math.min(times * 250, 2000) })
  ...
  async setOnce(key: string, ttlSeconds: number) {
    if (!this.client) return null
    try { return (await this.client.set(key, '1', 'EX', ttlSeconds, 'NX')) === 'OK' } catch { return null }
  }
